    ...
    public class ReportController : ControllerBase
    {
        ...
        private async Task<IEnumerable<ReportCategoryFullDetail>> GetCategoryDetailsAsync(string periodPattern, DateTime dateFrom, DateTime dateTo)
        {
            ...
            var materializedItemsSecondLevel = await _context.MaterializedMoneyItems
                ...
                .SelectMany(x => x.Subitems, (moneyItem, subitem) => new ReportCategoryFullDetail { Type = "MaterL2", ID = subitem.ID, Date = moneyItem.Date, Period = moneyItem.Date.ToString(periodPattern), Description = subitem.Note, CategoryID = subitem.CategoryID, Amount = subitem.Amount })
